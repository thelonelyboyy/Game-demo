...
更新时间：2026-07-14（灵根牌池、卡牌机制、地图与战斗 UI 收束）
...
- `run_godot_checks.py`：2026-07-14 最近一次全量运行 **36 项检查全部通过**，覆盖主流程、卡牌生命周期与牌库操控、三流派自动战斗/Boss 矩阵、法宝生命周期/手牌状态/牌组结构、状态法障、战斗减益与反制、三章内容、地图、普通/精英/Boss 战术、心魔难度、奖励经济、事件、合炼、战绩和篝火选择。本次元素圆章按用户要求未重复运行测试。
...
## 62. 三件通用首领法宝（2026-07-13）

- 奖励法宝池由 61 件扩至 64 件，首领档由 9 件扩至 12 件；新增三件均为全职业通用，只进入 Boss 三选一，不会污染普通、精英、宝箱或商店奖励。魔修专属仍为 17 件。
- 「无相天书」每回合额外抽 1 张牌，但每场战斗把 2 张虚无状态牌「蚀日伤痕」加入抽牌堆；污染只存在于本场战斗，不写入永久牌组，形成稳定过牌与抽牌质量之间的取舍。
- 「吞天气海」在角色灵力重置后每回合额外获得 1 点灵力，但所有敌人开战获得 2 层劲气；收益覆盖整场战斗，代价会随多敌人和召唤遭遇放大。
- 「归藏玉玺」在回合结束记录最多 2 点未用灵力，并在下一回合基础灵力重置后返还；储存值每次返还后清零且开战重置，支持防守回合为爆发回合蓄力。
- 新增 `boss-relic` 真实战斗 smoke，验证天书首回合多抽与两张污染、气海敌方开局劲气与逐回合加费、玉玺保留上限/单次消费/事件连接，以及三件资源的首领稀有度、跨职业可用和奖励池接入；奖励经济同步验证 64 件总池与 12 件首领档。全量检查由 25 项增至 26 项。

## 63. 虚弱与碎甲战斗减益（2026-07-13）

- 新增持续时间型负面状态「虚弱」与「碎甲」：虚弱令造成伤害降低 25%，碎甲令获得护体降低 25%；均在施加时立即生效、同名重复施加延长持续时间、归零时移除对应 Modifier，并明确参与 `is_debuff`/法障语义。
- 两种状态使用 2 点初始持续时间来覆盖一个完整玩家回合：敌方行动后施加，下一次玩家回合开始降为 1 并保持生效，再下次回合开始归零。意图统一写明「虚弱1回合 / 碎甲1回合」，玩家可在出牌前规划爆发或防守窗口。
- 新增共享 `attack_and_status_action.gd`，统一处理敌方伤害倍率、玩家受伤修正、攻击位移、减益施加和提示。第一章锁灵灯童的噬灵施加虚弱，第二章噬经毒蛾的毒粉施加碎甲，第三章业债判吏的判罚施加碎甲，幽冥判官的魂击施加虚弱；毒粉与判罚各下调 2 点基础伤害抵偿新增持续压力。
- 新增 `combat-debuff` 真实战斗 smoke：实际执行锁灵灯童动作，验证 12 点伤害与虚弱落地；再用正式打击/防御牌验证 `6→4` 伤害、`5→3` 护体的向下取整，覆盖重复延长、归零清理、Modifier 复原及四个敌人 AI 的减益/意图配置。状态法障、普通战术与 9 Boss 专项同步通过，全量检查由 26 项增至 27 项。

## 64. 负面状态反制链（2026-07-13）

- `StatusHandler` 新增通用负面状态计数、按数量净化和指定状态移除接口；持续时间型状态归零、强度型状态清空层数，非叠层状态直接移除。新增 `ConfiguredCleanseDebuffEffect`，`amount=0` 表示全部净化，正数表示最多移除数量，并可随卡牌突破成长。
- 魔修技能「焚秽诀」现在先净化 1 项负面状态，再获得 5 护体并焚化手中状态/诅咒牌换护体；突破后净化 2 项、基础护体 8、每张污染牌 6。净化被放在护体前结算，因此移除碎甲后本牌护体不会继续受旧修正影响。
- 既有少见丹药「清心丹」从单纯抽 3 重做为移除全部负面状态并抽 2，保持 26 种消耗品总量和原奖励稀有度分层；它现在是稀缺的一次性全清场，而非另一件普通过牌消耗品。
- 新增稀有通用法宝「澄神玉」：每场战斗开始获得 1 层法障，提供预防式反制；奖励法宝池由 64 件扩至 65 件，首领档仍为 12、魔修专属仍为 17，普通/精英/宝箱/商店按既有稀有权重自然获取。
- 修复 `Modifier.remove_value()` 延迟释放导致的单帧残留：修正节点现在先立即脱离 Modifier 树再释放，保证同一张牌内“先净化、后护体/伤害”读取新状态，`clear_values()` 同步遵守该时序。
- 新增 `debuff-counterplay` 真实战斗 smoke，覆盖澄神玉抵消第一项减益、焚秽诀基础/突破净化容量与护体顺序、清心丹通过正式药囊入口全清并抽 2，以及法宝/丹药奖励池接入；卡牌机制、魔修牌组、战斗减益、状态法障、通用法宝、法宝经济和丹药经济专项同步通过。全量检查由 27 项增至 28 项。

## 65. 卡牌生命周期实战回归（2026-07-13）

- 弃牌触发、消耗触发、成长和固定卡池发现沿用现有数据驱动机制与四张魔修示例牌：血偿、归煞、血祭斩、窥心魔典；四张牌均已进入魔修奖励池，发现池固定为 7 张禁术。
- 新增 `card-lifecycle` 真实战斗 smoke：血偿从手牌弃置后获得 4 护体并进入弃牌堆；归煞消耗后返 1 灵力且只进入消耗区；血祭斩成长值跨弃牌、洗牌、再抽取保持，但不会改写永久牌资源。
- 同一 smoke 自动处理突破后窥心魔典的二选结果，验证发现牌取消临时标记、加入真实手牌、弃置、洗回抽牌堆并再次抽到，确保它们只在本场战斗内持续参与抽弃循环。全量检查由 28 项增至 29 项。

## 66. 生命周期构筑法宝（2026-07-13）

- 新增 5 件通用法宝，把弃牌、消耗、成长和发现从单卡关键词扩展为可围绕构筑的奖励轴：普通「弃锋玉环」每回合首次从手牌弃置获得 4 护体；少见「焚简墨斗」每回合首次消耗获得 6 护体；少见「养刃灵石」令每回合首张成长牌在效果结算前额外成长一次。
- 稀有「万象残签」令每回合首次发现额外选择 1 张；稀有「点金书蠹」令每回合首次发现选中的 1 张可突破牌立即突破。两者共享 `discovery_guidance` 互斥组，同一奖励候选与同一局持有均不能叠加。
- 新增独立语义事件 `card_discarded_from_hand`，只在牌真实进入弃牌堆后触发，不依赖飞行动画是否播放；发现请求新增 `bonus_upgrade_count`，在牌加入手牌前完成突破，卡面和后续抽弃循环读取的都是最终状态。
- 奖励法宝池由 65 件扩至 70 件，首领档仍为 12、魔修专属仍为 17；新增 `lifecycle-relic` 真实战斗 smoke 覆盖五件法宝数值、每回合重置、互斥、额外选择和突破结算。全量检查由 29 项增至 30 项。

## 67. 手牌状态构筑法宝（2026-07-13）

- 新增 5 件通用法宝：少见「留锋剑穗」在回合结束按手中保留牌获得每张 3 护体、最多 3 张；少见「镇魔经夹」按状态/诅咒牌获得每张 4 护体、最多 2 张；普通「守一铜印」在灵力恰好耗尽时获得 5 护体。
- 少见「涤尘玉页」每回合首次抽到状态牌或诅咒牌时补抽 1，触发守卫在补抽前设置，连续污染牌不会递归无限抽牌；稀有「幻生书签」每回合首次打出临时牌时抽 1，使英雄技能与临时牌构筑拥有稳定法宝收益。
- 回合结束法宝在正式弃牌阶段前读取真实手牌，保留与污染防守能覆盖随后敌方回合；所有 5 件均为通用非首领档，按既有章节稀有度权重进入普通、精英、宝箱和商店来源。
- 奖励法宝池由 70 件扩至 75 件，首领档仍为 12、魔修专属仍为 17；新增 `hand-state-relic` 真实战斗 smoke 覆盖手牌分类、计数上限、空灵力条件、污染补抽防递归和临时牌每回合重置。全量检查由 30 项增至 31 项。

## 68. 牌组结构与出牌组合型法宝（2026-07-13）

- 新增 5 件通用法宝并完成路线图的 80 件数量目标：稀有「空明玉简」在永久牌组不超过 12 张时开战获得 2 法障；少见「万卷经匣」在永久牌组至少 20 张时开战获得 15 护体。两者共享 `deck_shape` 互斥组，奖励和直接装备均不能同时持有。
- 稀有「三相熔炉」每回合首次打齐攻击、技能、功法三种类型后获得 1 灵力与 6 护体；少见「轻身算珠」在每回合第 3 张 0 费牌时抽 1；普通「百炼经印」在每回合首张已突破牌时获得 5 护体。
- 牌组规模读取永久 `CharacterStats.deck`，发生在开战抽牌堆复制之前，不会被战斗内临时牌、污染牌或发现牌误改；组合型法宝都在 `player_turn_started` 清理计数，并在奖励发放前设置触发锁，避免补抽或返灵力造成重复结算。
- 新增 `deck-shape-relic` 真实战斗 smoke，覆盖 12/20 张边界、规模法宝互斥、三类型组合、零费第三张、突破牌与跨回合重置；奖励法宝池由 75 件扩至 80 件，首领档仍为 12、魔修专属仍为 17。全量检查由 31 项增至 32 项。

## 69. 抽牌与滞留污染生命周期（2026-07-13）

- `Card.LifecycleTrigger` 新增 `DRAWN` 与 `TURN_ENDED_IN_HAND`，`CultivationCard` 对应新增 `draw_trigger_effects` / `end_turn_trigger_effects` 数据数组；卡面关键词统一使用「抽牌触发」与「滞留触发」，默认 tooltip 会解释精确结算时机。
- 抽牌触发只在 `PlayerHandler.draw_card()` 将牌从抽牌堆成功抽入手牌后执行；发现、英雄技能临时发牌和跨牌堆检索不会误算为抽牌。滞留触发在回合末弃牌裁决前扫描真实手牌，因此临时、虚无、保留和普通弃牌都先触发，再各自进入原有生命周期。
- 新增状态牌「封灵符」：抽到时失去 1 灵力，回合结束虚无消散；新增状态牌「劫火烙」：回合结束仍在手中则失去 2 生命，随后正常进入弃牌堆并继续循环。两张牌都复用高分辨率魔修插画，不引入低清占位图。
- 第一章灰经邪僧的首个污染动作改为向抽牌堆混入封灵符，护体污染动作改为向弃牌堆混入劫火烙，意图明确写出牌名；它现在同时考验当回合灵力规划与长期污染净化，而非两次写入同一种纯占位状态。
- `ConfiguredManaLossEffect` 提供可复用的正数扣灵力效果；自动合炼同步合并新增两类生命周期效果。`PlayerHandler` 在玩家死亡时立即停止抽弃 tween 和回合推进，防止抽牌/滞留反噬致死后仍开始敌方回合。
- `card-lifecycle` 真实战斗 smoke 新增封灵符从抽牌堆入手扣费、劫火烙回合末扣血后进入弃牌堆、灰经邪僧动作引用验证；`normal-tactics` 同步按新污染牌检查，关联卡牌、融合与战术测试均通过。

## 70. 牌顶过滤与主动弃牌闭环（2026-07-13）

- 新增 `ConfiguredTopdeckFilterEffect`：按原顺序检视抽牌堆顶 N 张，只把其中状态牌/诅咒牌送入弃牌堆，未过滤牌相对顺序不变；过滤不是抽牌或从手牌弃置，因此不会提前触发封灵符扣费、劫火烙反噬或弃牌生命周期。
- 新增 `ConfiguredDiscardRandomBlockEffect`：随机弃置手牌中除来源牌外的指定数量，只有实际弃置成功才按张获得护体；弃置统一走 `PlayerHandler.discard_card_from_hand()`，所以血偿、血债等弃牌触发与法宝计数都照常生效。
- 新增三张魔修奖励牌：白卡「滤念诀」0 费消耗，检视牌顶 4 张、过滤至多 2 张污染并抽 1；白卡「弃念护脉」0 费，随机弃 1 张其它牌并获得 6 护体；蓝卡「灰烬回照」1 费消耗，检视牌顶 6 张、过滤全部污染并抽 2。三张均可突破，弃念护脉护体 6→9，其余两张提高检视和抽牌量。
- 魔修奖励池由 95 条/88 个唯一 ID 增至 98 条/91 个唯一 ID；当前白 34、蓝 34、金 18、暗金 5，攻击 37、技能 40、功法 14，仍保持 2 张 X 费与四路线分档供给。`DEMONIC_BALANCE.md` 已重新生成并通过全部结构门槛。
- 新增第 33 项 `pile-control` 真实战斗 smoke：固定牌顶验证限量/不限量过滤、未过滤顺序、污染不提前触发；再以血债作为唯一随机弃牌目标，验证反噬、入弃牌堆、来源牌排除与成功后 6 护体。卡牌基础、奖励批量模拟和魔修套件同步通过。

## 71. 三流派真实自动对战基线（2026-07-13）

- 新增可复用 `BattleAutoplayer`：只读取正式手牌和当前灵力，直接调用 `Card.play()`，单体牌选择存活敌人，功法优先、防御牌随敌方意图提高优先级，魔焰按配色顺序施放；出牌、伤害、状态、法宝、牌堆、敌方行动和胜负全部走真实战斗逻辑。
- 新增 `archetype_autoplay_smoke.gd/.tscn`，固定流派/遭遇种子，以血契煞气、魂印和魔焰三套成型牌组分别挑战铜傀、玄铁傀儡和冥骨龙，共 9 场真实对局。失败信息会输出逐回合卡牌、玩家生命/护体、敌方意图和剩余生命，便于定位构筑或数值断点。
...
## 73. 2026-07-14 最新 UI、地图、卡牌机制与灵根交接

- 地图层数已扩为 20 层；前段普通怪比例经过来回微调，保留足够战斗密度并增加事件、商店、祝福等非战斗节点。地图图例进入地图即显示，节点含义无需先选择第一层；默认连接线为黑色，走过路径为红色。
- 战斗人物/怪物外层冗余边框已移除。人物卡贴近屏幕底部并预留两行状态空间；结束回合位于人物卡左侧且放大；英雄技能缩名为「焚心」并置于人物卡左上方。人物/怪物插画区域悬停显示名称和介绍，状态图标已放大。
- 玩家与怪物血量数字位于红条下方，护体数字位于蓝条下方；插画下方只显示真实状态，不再常驻「护体 0」。状态支持悬停词条说明。
- 卡牌生命周期已覆盖临时、消耗、保留、固有、虚无，以及弃牌触发、消耗触发、成长和固定卡池发现。英雄技能「焚心」自损 2 点并生成一张随机非攻防魔修临时牌，临时牌使用或回合结束均移除，不进入抽弃循环。
- 魔修灵根选择固定提供金、水、火。魔修职业奖励池按显示名称稳定分入无属性、金、水、火四组，每组 22 个权重条目；同名权重牌不会跨组。规则在新开局、继续旧存档与图鉴收集前都会刷新。
- 顶栏原灵根徽章改为展示牌组内数量最多的元素与张数；并列时优先已选灵根，否则按固定顺序稳定决胜。实际灵根成长规则仍保留在 tooltip 中。
- 新增 `CardChangeFeedback` 全屏确认层。开局灵根赠送职业牌，以及随机事件/祝福中的获得、移除、突破、复制和诅咒结果都会展示完整卡面，必须点击「确认」后继续；多条结果自动排队。
- `CardVisuals` 右上元素标记新增五行专属色圆章：金 `d9c77a`、木 `79b66a`、水 `6fb2d8`、火 `e06a3b`、土 `b99358`。不要在各场景重复实现，继续复用 `SpiritRootText.element_color()` 与 `_apply_element_badge_style()`。
- 本批改动提交前最近一次完整回归为 36/36 通过；新增圆章按用户要求未运行测试。若下一会话继续视觉调整，优先在 1920×1080 下检查战斗手牌、奖励三选一、商店和卡牌确认弹窗四种尺寸。
