...
更新时间：2026-07-23（正式美术接入、商店/战斗 UI 收束与提交发布）
...
- **2026-07-23 正式美术批次完成接入**：本轮共有 **157 项正式资源通过校验**，包括 42 张战斗角色/怪物卡图、50 张魔修与通用卡牌插画、42 枚法宝图标、3 张事件插画、4 张背景和 16 枚意图/状态图标。敌我 `Stats` 已支持独立 `battle_art`，运行时不再引用根目录 `test1.png` / `test2.png`。生产清单与验收状态见 `game-demo/art/imagegen_production_manifest.md` 和 `game-demo/art/q_card_imagegen_manifest.md`。
- **2026-07-23 美术遗留口径**：瘴毒蛾新图因伪文字和烘焙边框问题未接入，继续使用旧图；体修/驭兽第二批卡图共 40 项按用户要求暂停，其中已生成的体修草稿保留在素材目录但没有运行时引用。不要把“暂停”误记为资源缺失或接入完成。
- **2026-07-23 商店界面重构收束**：商店明确划分卡牌、法宝、符箓丹药、删牌四区；符箓丹药货位增至 4 个，删牌与离开按钮放大，标题说明上移，并改用已有美术切片装饰各区边框，避免程序线框与内容拥挤。
- **2026-07-23 战斗界面收束**：结束回合按钮已移到焰轮正上方并按真实最小尺寸居中；同一角色的扣盾、扣血、自损和回血数字继续严格串行，前一条完整播放并释放后才显示下一条。
- **2026-07-23 卡牌框方案回退**：当天尝试的两版模块化新卡框及生成素材均因视觉效果未达标而完整撤回。当前继续使用原有整张 `card_frame_*` 贴图与既有 `CardVisuals` 结构，未保留 `cards_v2` 运行时引用；后续若再次重构，必须先做可审阅的静态样稿并确认方向，再修改游戏结构。
- **2026-07-23 发布检查与竞态修复**：发布前检查发现旧工作簿曾把原有及新差异化卡牌的“消耗”开关回写为关闭；现已恢复原有 22 张消耗牌、保留新增爆发/检索消耗牌，并从资源重新生成 `game_data.xlsx`，当前共 40 张消耗牌。完整矩阵首次 39/42 通过，修复消耗断言和退出战斗时敌人意图强转已释放玩家的竞态后，失败项全部定向通过；`run-flow` 连续 3 次、Boss 与结束回合交互复测均通过，当前有效基线为 **42/42**。
...
- **2026-07-16 战斗数值反馈严格串行**：同一角色的护体损失、生命损失、自损与治疗改为等待前一条完整播放并释放后再开始，修复 Tween 等待段被错误并行、导致数字仍提前出现的问题。单条飘字生命周期收紧至约 0.91 秒，并保留不同角色之间并行反馈，群攻不会逐个排队；人物卡护体条与生命条继续按数值快照串行。`combat_feedback_sequence_smoke` 现同时验证预约时间和实际可见/释放顺序。
- **2026-07-16 调试控制台升级**：旧命令保持兼容，新增 `status` 运行摘要、手牌增删/抽弃、护体、灵力、敌人伤害/击杀/意图刷新、回合状态与操作恢复、地图解锁/自由导航、45 个事件检索与直达、心魔难度、手动保存和资源索引刷新。卡牌/法宝/丹药库改为 ID+中文名的去重列表并支持模糊筛选；常用根命令和子命令支持中文。输入框新增 ↑↓ 历史、Tab 补全、Ctrl+L 清屏、Esc 关闭；`debug_console_smoke` 覆盖中文命令、现行难度、旧伤害钩子、全事件索引、历史与补全。
...
- **调试控制台已更新**：Run 场景内按 **Ctrl + 反引号** 打开，不使用 F8（Godot 编辑器里 F8 会停止运行）。输入 `help` 查看完整命令、输入 `status` 查看当前运行摘要；卡牌、手牌、生命/护体/灵力、金币、法宝、丹药、敌人、回合、地图、事件、难度与下次伤害均可直接调试，并支持中文别名、历史与补全。根目录 `DEBUG_CONSOLE_GUIDE.md` 是完整中文使用手册。
...
- `validate_project.py` 最近有效基线为 **0 error / 0 warning**；校验脚本已修掉「默认值字段被当成缺失」的误报，不再有历史的 198 个假错。
...
│  │  ├─ cards\<职业>\ai\      卡牌大插画（魔修/通用本轮已统一接入；体修/驭兽新批次暂停）
│  │  ├─ combatants\           玩家与全部怪物的战斗卡图
│  │  ├─ relics\icons\         法宝独立图标
│  │  ├─ icons\                意图、状态等正式图标
...
- `validate_project.py`：2026-07-23 发布前复跑为 0 error / 0 warning。
- `validate_card_standardization.py`：192 张卡牌、340 个效果组件、419 条数值参数通过；当前关键词包含 40 张消耗牌。
- `test_game_data_editing.py`：突破回写、融合新增/停用均通过；`game_data.xlsx` 已从最终资源重新导出。
- `run_godot_checks.py`：2026-07-23 当前有效基线为 **42/42**。完整矩阵首次 39/42，暴露消耗关键词被旧工作簿覆盖及 `run-flow` 退出战斗时的已释放玩家强转竞态；修复后 `demonic-card-suite`、`event-progression`、`card-keyword-standardization` 均通过，`run-flow` 连续 3 次通过，Boss 与结束回合交互同步通过。后续常规修改仍按上方约定只跑定向测试。
...
- **调试控制台**：`scenes/debug/debug_console.gd` 与 `custom_resources/debug_console_state.gd`，Run 场景按 Ctrl + 反引号打开。除增删/突破/融合卡牌、生命/金币/法宝/丹药与下次伤害外，现已覆盖手牌、护体、灵力、敌人、回合恢复、地图、事件、难度、手动保存及运行摘要；伤害测试钩子接在 `card_effect.gd`、`configured_flame_effect.gd`、`configured_soul_mark_detonate_effect.gd`、`player.gd`。
...
- `game_data.xlsx`（统一数值总表，项目根）——数据页：**卡牌 / 卡牌效果 / 关键词标准 / 效果标准 / 融合配方 / 祝福 / 事件 / 事件灵根 / 符箓丹药 / 法宝 / 法宝参数 / 怪物 / 怪物行动 / 关卡设计 / 商店价格** + 说明页。只改浅黄列，灰列只读。
...
- 模块：`scripts/card_table.py`（卡牌）/ `card_effect_table.py`（效果组件）/ `relic_table.py`（86 件法宝与 148 行参数）/ `blessing_table.py`（祝福）/ `event_table.py`（45 个 GenericEvent 与 3 个灵根分支）/ `potion_table.py`（26 个符箓丹药）/ `enemy_table.py`（39 个怪物）/ `ai_table.py`（27 套 AI、83 个行动）/ `level_table.py`（51 个在用关卡）/ `shop_price_table.py`（卡牌、法宝、丹药售价与删牌服务）。
...
- **可调范围**：「卡牌」新增流派标签只读列、成长触发/成长值/成长上限可调列；「卡牌效果」覆盖数值参数并显示连携等效果条件；「法宝参数」可直接调整全部已暴露数值与“每回合限一次”等布尔项。效果类型、目标模式、魔焰颜色/附加类型等结构枚举仍在 Godot 改。「融合配方」的动态合成行可调结果名与结果费用；固定成品卡在「卡牌」「卡牌效果」调。关卡页暂不直接增减敌人槽位。全表无修改回写 = 改 0 文件（2026-07-15 已用 openpyxl、Excel COM 和回写脚本验证），中文反向回写亦已覆盖多表变更测试。
...
- 2026-07-23 正式生产批次验收 **157 项**：42 张战斗角色/怪物卡图、50 张魔修与通用卡牌插画、42 枚法宝图标、3 张事件插画、4 张背景、16 枚意图/状态图标。详细逐项状态以 `art/imagegen_production_manifest.md` 为准。
- 魔修与通用卡牌的新插画放在 `art/cards/demonic_cultivator/ai/`、`art/cards/common/ai/` 并已接入资源；继续出图时要保持当前魔修角色、色彩和无文字纯插画口径。
- 体修/驭兽新卡图批次共 40 项已暂停。素材目录中少量体修草稿没有运行时引用，恢复该批次前先重新确认美术方向，不要直接批量接线。
- 42 个已验收战斗单位通过独立 `battle_art` 显示正式战斗卡图；瘴毒蛾新图因伪文字/烘焙边框未通过，仍使用旧图，这是当前唯一真正阻塞的单项资源。
- 42 件法宝已接独立图标；事件、背景及意图/状态图标的接线与 QA 记录均保留在 `art/qa/` 和生产清单中。
- 当前卡框仍为原有 AI 整框贴图方案。2026-07-23 两版模块化重构已撤回，后续不要在没有确认静态样稿的情况下恢复 `cards_v2` 或再次拆分费用、稀有度、元素、类别组件。
- `art/tiles/tile_*` 中未被正式图标替换的资源仍是 16px 占位像素图（**不要给它们开 mipmap**）。
...
- **正式美术仍需整局人工 QA**：自动资源审计和无头加载可确认引用、导入与脚本安全，但不能代替实际构图检查。下一轮应完整跑一局，重点查看不同敌人组合、卡牌 hover/拖拽、商店四区和事件插画的裁切与可读性。
- **瘴毒蛾新图暂缓接入**：多次生成结果仍带伪文字或烘焙卡框，当前保留旧图。重新制作时只生成无边框、无文字的纯角色插画。
- **卡牌框重构已撤回**：当前没有 `cards_v2` 方案需要继续开发；再次启动前先做静态视觉方案并确认费用、元素、稀有度、类别四类信息层级。
...
- **魔修待办**：重名、早期血祭强度、魔焰 rider 和魂印 payoff 已处理；2026-07-14 最新牌池按无属性/金/水/火四组各 22 个权重条目重分组。正式插画已完成本批接入，后续重点改为完整流程实战平衡、卡牌使用率分析与数值微调，数量口径以 `DEMONIC_BALANCE.md` 和实际资源为准。
...
- 主菜单标题已烤进背景图 `art/backgrounds/background1.png`（旧 `baijie_chengxian_title.png` 已删）；战斗与选人背景已迁入正式美术目录，运行时不再依赖项目根 `test1.png` / `test2.png`。

## 9. 后续建议

1. 优先完整试玩魔修单职业流程：战斗、事件、商店、宝箱、篝火、Boss、失败/胜利和失败遗产都要覆盖，并集中记录正式美术的裁切、辨识度和风格问题。
2. 按实感微调卡牌/法宝/事件平衡、战斗动效节奏、SFX/BGM 音量及地图/商店/奖励界面的视觉密度；优先通过 `game_data.xlsx` 调整，避免重新写死数值。
3. 单独重做瘴毒蛾纯插画；体修/驭兽新卡图保持暂停，除非用户明确恢复该批次。
4. 卡牌框如需重构，先交付静态样稿确认费用、元素、稀有度和类别的视觉层级；未确认前保持现有 `card_frame_*` 整框结构。
5. 对卡牌标题字体、描述字体补充更有国风感的正式字体资源。
6. 常规改动遵循“改哪里、测哪里”；系统重构、跨模块大改或重要发布再运行完整 `run_godot_checks.py`，并人工进入战斗、商店、地图、奖励、主菜单确认视觉效果。
...
- 当时全量回归为 36/36 通过；2026-07-15 差异化重构后的最新覆盖口径已扩展为 41/41，详见下方“卡牌、法宝与事件差异化重构”。
...
### 2026-07-15 卡牌、法宝与事件差异化重构

- 设计参考只取“机制层”而未照搬具体内容：采用独特被动改变决策、卡组/装备协同、牌序与元素组合、显式收益—代价等原则，全部名称、数值、职业主题和实现均按本项目重写。
- 新增通用卡牌机制：
  - **连携**：标准效果可读取本回合上一张牌的类型或五行，满足条件后追加 `bonus_amount`；条件说明会进入卡面动态 tooltip 与 `game_data`。
  - **蓄势**：保留牌在回合结束仍留手时成长，支持每次成长值和本场成长上限；结算顺序为滞留效果 → 成长 → 保留。
  - 效果条件另支持本回合出牌数和低生命阈值，为后续卡牌继续扩展留出标准接口。
- 卡牌差异化脚本：`scripts/rebalance_distinctive_content.py` 按原效果结构识别同构组，并用蓄势、先手、爆发消耗、周天、类型/五行连携、检索与取回形成不同职责；`scripts/card_balance_targets.json` + `rebalance_card_values.py` 保存绝对数值目标，避免重复运行累计缩放。
- 当前 192 张卡牌形成 190 个“类型/费用/关键词/成长/效果条件”设计签名；原先 33 组明显同构已收敛到 2 组二张牌的轻度相似。104 张同构卡完成玩法分流，67 个数值目标中仅“爆发消耗”保留主动上调，固有/周天不再额外削弱基础数值。
- 职业定位：体修为承伤转化/金身；剑修为剑式连携/铸剑；魔修保留血炼、魂印、魔焰、煞气四线；驭兽为召唤/兽群。`game_data` 的“流派标签”列可直接查看，不再只看名字猜用途。
- 事件新增百分比治疗/失血、按已损生命或牌组规模获得灵石、净化污染、随机重铸、随机拓印、多张突破等标准效果。44 个事件改写核心选项，45 个事件现有结构全部不同；组合效果仍使用 `|` 顺序结算。
- 15 件代表性法宝完成数值与说明重做：明确“第一轮防守、主动承伤、精确耗尽、保留牌、剩余灵气、群伤清场、长期经济”等不同用途。86 件法宝和 148 行可调参数已全部同步到 `game_data.xlsx`。
- 奖励协同判定加入卡牌流派缓存；卡牌奖励大样本 smoke 从超过 90 秒降至约 10 秒，结果分布不变。
- 验证：`validate_project.py` 0 error / 0 warning；卡牌标准化 192 张、340 个效果组件、419 行数值参数通过；`game_data.xlsx` 16 页全部渲染、公式错误 0、无修改回写 0 文件，并经 Excel COM 原生打开无修复提示。全量回归清单 41/41 已覆盖：首次全跑后，对 8 个受新数值影响的失败项修正并逐项定向复测通过。
- 后续平衡建议：优先人工完整跑魔修魂印、爆发消耗、保留蓄势各一局，再根据选牌率/胜率微调 `game_data`；不要仅按单卡理论费用继续全局百分比缩放，否则会重新抹平差异。

### 2026-07-23 正式美术接入与界面收束

- 正式美术生产清单完成 157 项验收与接线：42 张战斗角色/怪物卡图、50 张魔修与通用卡牌插画、42 枚法宝图标、3 张事件插画、4 张背景、16 枚意图/状态图标；资源清单、提示词、引用和 QA 结果集中保存在 `game-demo/art/imagegen_production_manifest.md`、`game-demo/art/q_card_imagegen_manifest.md` 与 `game-demo/art/qa/`。
- 战斗单位资源新增独立 `battle_art` 展示链路，移除运行时对项目根测试背景的依赖；卡牌与法宝资源批量接入独立正式图，不再以少量共享占位图承担主要视觉。
- 商店重构为卡牌、法宝、符箓丹药、删牌四个明确区域，符箓丹药增至 4 个货位，离开/删牌操作放大，并复用现有美术切片构成区域边框。
- 结束回合按钮移动到焰轮正上方；战斗飘字、生命条与护体条的同目标反馈保持严格串行。
- 当天两次卡牌框模块化尝试均因视觉结果未通过而完整撤回，旧整框素材和结构保留。该回退是最终决策，不应从已删除的生成资源继续实现。
- 体修/驭兽第二批卡图按用户要求暂停；瘴毒蛾新图因伪文字和烘焙边框未通过，二者均已在清单中明确标注，避免后续误接。
- 发布前完整矩阵发现旧 `game_data.xlsx` 曾把卡牌差异化脚本生成的消耗开关以及原有消耗牌一并回写为关闭；已恢复原有 22 张消耗牌并保留新规则牌，从最终资源重建工作簿，当前卡牌标准化统计为 40 张消耗牌。
- `run-flow` 的历史异步竞态最终定位为旧战斗退出时抽牌补间仍发出全局信号，敌人意图刷新随后强转已释放的玩家对象；`Enemy.update_intent()`、`EnemyAction` 完成回调及螃蟹攻击数值计算现均先检查实例有效性。修复后流程连续 3 次通过，当前有效回归基线为 42/42。
