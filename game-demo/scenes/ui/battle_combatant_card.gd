...
func _refresh_bars() -> void:
	...
	_set_vertical_fill(_block_fill, block_ratio)
...
func _refresh_intent() -> void:
	...
	_intent_label.text = category_text if value_text.is_empty() else "%s %s" % [category_text, value_text]
...
func _make_status_chip(status: Status) -> Control:
	...
	label.text = str(status.stacks if status.stack_type == Status.StackType.INTENSITY else status.duration)
...
func _portrait_texture() -> Texture2D:
	...
	if stats is EnemyStats and (stats as EnemyStats).id == "bull_demon":
